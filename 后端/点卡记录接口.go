...
// 点卡端_查询点卡流水只允许当前点卡读取自己的流水。设备 ID 和别名已经
// 合并到 remark，持卡用户看到的每一行正好对应一次余额变化。
func 点卡端_查询点卡流水(ctx *gin.Context) {
	...
	tableName, err := 点卡数据表名(userContext.Name)
	if err != nil {
		失败提示(ctx, err.Error())
		return
	}
	...
	rows, total, err := 查询点卡流水记录(db_point_ledger.Where("admin = ? AND card = ?", userContext.Name, card), page, pageSize)
	if err != nil {
		失败提示(ctx, "查询点数流水失败")
		return
	}
	成功提示(ctx, gin.H{
		"data": 点卡流水展示列表(rows, false), "num": total,
