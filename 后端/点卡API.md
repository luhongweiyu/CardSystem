...
如果推断截止晚于旧 `authorized_until` 且当前时间尚未超过推断截止，视为设备可能仍在线，从旧截止时间续一个授权时长并扣一次点。超过推断窗口后，实际收到登录或心跳就从当前时间开始新的授权时长；后台清理在没有新请求时会删除该离线会话。后台清理续费时若余额不足或计费方案不可用，也会删除已过期会话，客户端需重新登录。三条路径采用同一续费起点规则。
