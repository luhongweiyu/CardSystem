  ...
  <section class="页面" v-loading="加载中">
    ...
    <el-card shadow="never" class="筛选卡片">
      <el-form :inline="true" @submit.prevent>
        ...
        <el-form-item label="点卡卡密">
          <el-input
            v-model="筛选.card"
            clearable
            placeholder="支持模糊搜索"
            style="width: 210px"
            @keyup.enter="查询点卡卡密(true)"
          />
        </el-form-item>
        ...
        <el-form-item>
          <el-button type="primary" @click="查询点卡卡密(true)">查询</el-button>
          <el-button @click="重置筛选">重置</el-button>
        </el-form-item>
      </el-form>
      <div class="批量操作">
        <span>已选 {{ 已选点卡卡密.length }} 张</span>
        <el-button size="small" type="warning" :disabled="!已选点卡卡密.length" @click="批量修改状态(4)">
          冻结
        </el-button>
        <el-button size="small" type="success" :disabled="!已选点卡卡密.length" @click="批量修改状态(2)">
          解冻
        </el-button>
    ...
    <!-- 生成点卡卡密 -->
    <el-dialog v-model="生成框.显示" title="生成点卡卡密" width="560px" destroy-on-close>
      <el-form label-width="100px" v-loading="生成框.加载中">
        ...
        <el-form-item label="生成方式">
          <el-radio-group v-model="生成框.random">
            ...
            <el-radio :label="false">指定点卡卡密</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="!生成框.random" label="点卡卡密内容" required>
...
const 查询点卡卡密 = function (resetPage = false) {
  ...
  return post('/point_card/list', {
    ...
    .then((res) => {
      if (!res.data?.state) throw new Error(res.data?.msg || '查询点卡卡密失败')
      点卡卡密列表.value = res.data.data || []
...
const 点卡卡密排序变化 = function ({ prop, order }) {
...
const 重置筛选 = function () {
  ...
  点卡卡密表格.value?.clearSort()
  查询点卡卡密(true)
}
const 选择变化 = (rows) => {
  已选点卡卡密.value = rows.map((row) => row.card)
}
...
const 批量修改状态 = function (state) {
  if (!已选点卡卡密.value.length) return
  post('/point_card/state', { cards: 已选点卡卡密.value, card_state: state })
    .then((res) => {
...
const 删除单张 = function (row) {
  ElMessageBox.confirm(`确定删除点卡卡密 ${row.card}？删除后同名卡密可以再次生成，流水仅保留最近30天。`, '确认删除', {
    ...
    .then(() => post('/point_card/delete', { cards: [row.card] }))
    .then((res) => {
...
const 批量删除 = function () {
  ElMessageBox.confirm(`确定删除已选的 ${已选点卡卡密.value.length} 张点卡卡密？`, '确认删除', { type: 'warning' })
    .then(() => post('/point_card/delete', { cards: 已选点卡卡密.value }))
    .then((res) => {
      if (!res.data?.state) throw new Error(res.data?.msg || '删除失败')
...
const 查询流水 = function (resetPage = false) {
  ...
  return post('/point_card/ledger', { card: 流水框.card, page: 流水框.page, page_size: 流水框.page_size })
    .then((res) => {
...
const 导出当前页 = function () {
  const text = 点卡卡密列表.value.map((row) => row.card).join('\n')
  if (!text) return
