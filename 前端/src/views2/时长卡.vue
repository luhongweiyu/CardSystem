<template>
  <section class="页面" v-loading="加载中">
    <div class="页面标题行">
      <div>
        <h2>时长卡管理</h2>
        <p class="说明">时长卡独立于点卡；卡密首次登录激活，按卡面固定时长使用。</p>
      </div>
    ...
    <el-table :data="列表" border stripe row-key="card" @selection-change="选择变化" @sort-change="排序变化">
      ...
      <el-table-column label="操作" width="260" fixed="right">
        <template #default="scope">
          ...
          <el-button link type="success" @click="打开单张续费(scope.row)">续费</el-button>
    ...
    <el-dialog v-model="生成框.显示" title="生成时长卡" width="560px" destroy-on-close>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="生成">生成</el-button>
      </template>
    </el-dialog>
...
const 生成框 = reactive({ 显示: false, 加载中: false, software: 0, duration_minutes: 1440, num: 1, random: true, cards: '', latest_activation_days: -1, notes: '', config_content: '' })
...
const 状态类型 = (row) => ({ 未激活: 'info', 已激活: 'success', 已到期: 'warning', 冻结: 'danger' })[状态文本(row)] || 'info'
...
const 查询软件 = () => post('/user_query_soft_list', {}).then((res) => {
  ...
  if (!软件列表.value.some((item) => Number(item.ID) === Number(生成框.software))) 生成框.software = 软件列表.value[0]?.ID || 0
})
...
const 打开生成 = () => {
  Object.assign(生成框, { 显示: true, 加载中: false, software: 软件列表.value[0]?.ID || 0, duration_minutes: 1440, num: 1, random: true, cards: '', latest_activation_days: -1, notes: '', config_content: '' })
  生成结果.value = ''
}
const 生成 = () => {
  if (!生成框.software || !生成框.duration_minutes || !生成框.num) return ElMessage.warning('请选择软件并填写有效的时长和数量')
  if (!生成框.random && !生成框.cards.trim()) return ElMessage.warning('请输入指定卡密')
  生成框.加载中 = true
  const latestActivationMinutes = 生成框.latest_activation_days < 0 ? -1 : 生成框.latest_activation_days * 1440
  post('/duration_card/create', { ...生成框, latest_activation_minutes: latestActivationMinutes })
    .then((res) => {
...
onMounted(() => { Promise.all([查询软件(), 查询列表(true)]).catch(() => {}) })
</script>
