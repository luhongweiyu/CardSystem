<template>
  <section class="页面">
    <h2>接入帮助</h2>
    <p class="说明">客户端按“登录 → 定时心跳 → 退出”的生命周期使用点卡。</p>
    <el-card shadow="never" class="说明卡片">
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="心跳">
          POST /card/card_ping，提交登录返回的 needle；建议按软件设置的心跳周期发送。
        </el-descriptions-item>
        <el-descriptions-item label="退出">
          POST /card/card_logout，提交 software 与 device_id；可同时提交 needle
          做附加校验，退出后立即释放设备会话。
        </el-descriptions-item>
        <el-descriptions-item label="周期价格">
          GET/POST /card/period_prices，可读取当前软件启用的周期和扣点价格。
        </el-descriptions-item>
